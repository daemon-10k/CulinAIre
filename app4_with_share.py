...
def create_share_url(recipe_title):
    ...
    return f"http://localhost:8501/?recipe={encoded_title}"  # Replace with your deployed URL
...
if shared_title:
    shared_title = shared_title[0].replace('+', ' ')
    shared_title = urllib.parse.unquote(shared_title)
    ...
    if not matches.empty:
        ...
        with col1:
            ...
            with open(pdf_path, "rb") as f:
                st.download_button("📄 Download PDF", f.read(), file_name=f"{recipe['title']}.pdf", mime="application/pdf", key=f"dl_shared_{recipe['title']}")
        with col2:
            st.markdown(f"🔗 Shared via: `{shared_title}`")
    else:
        st.warning("⚠️ No recipe found matching the shared title.")
    st.stop()  # Stop further processing when a shared recipe is shown
# Safety check
